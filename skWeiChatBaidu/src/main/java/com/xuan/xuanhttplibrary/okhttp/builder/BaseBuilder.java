...
public abstract class BaseBuilder {
    ...
    public BaseBuilder addSecret() {
        ...
        params("lat", LocaleHelper.getLanguage(MyApplication.getContext()));
    ...
    public BaseBuilder addSecret(String payPassword, String money) {
        ...
        if (url.equals(mConfig.REDPACKET_SEND)
                || url.equals(mConfig.SKTRANSFER_SEND_TRANSFER) || url.equals(mConfig.RECHARGE_Circle)) {
            // 发红包 || 转账 调用的接口
            String mLoginUserId = CoreManager.requireSelf(MyApplication.getInstance()).getUserId();
        ...
        params("time", time);
        params("secret", secret);
        params("lat", LocaleHelper.getLanguage(MyApplication.getContext()));//LocaleHelper.getLanguage(MyApplication.getContext())
        return this;
    }
