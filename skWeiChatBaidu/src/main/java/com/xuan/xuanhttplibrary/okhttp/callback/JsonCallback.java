    ...
    public void onResponse(Call call, Response response) throws IOException {
        if (response.code() == 200) {
            try {
                ...
                errorData(call, new Exception(MyApplication.getInstance().getString(R.string.a78)));
            }
        } else {
            Log.i(HttpUtils.TAG, "服务器请求异常");
            errorData(call, new Exception(MyApplication.getInstance().getString(R.string.a81)));
        }
    }
