...
class ImVideoCallActivity : BaseActivity(), View.OnClickListener {
        ...
        if (!isRellyFinish && !isManual) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                if (FloatWindowUtil.checkFloatPermission(this@ImVideoCallActivity)) {
                    ...
                    ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f5))
                }
            } else {
                startVideoFloatWindow(true)
            }
    ...
    override fun onBackPressed() {
        ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f6))
    }
        ...
        when (v?.id) {
            R.id.img_answer -> {//接听
                if (NetworkUtils.isConnected()) {
                    ...
                    ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f7))
                    refuseCall()
                }
            }
            R.id.img_refuse -> {//拒接
            ...
            R.id.iv_float -> {//开启悬浮窗
                if (FloatWindowUtil.checkFloatPermission(this@ImVideoCallActivity)) {
                    ...
                    ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f8))
                }
            }
    ...
    private val mRtcEventHandler = object : IRtcEngineEventHandler() { // Tutorial Step 1
        ...
        override fun onConnectionLost() {
            ...
            ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f9))
            mRtcEngine?.leaveChannel()
        }
    ...
    private var task: TimerTask = object : TimerTask() {
        override fun run() {
            ...
            runOnUiThread {
                if (!JitsistateMachine.isIncall) {
                    mRtcEngine?.leaveChannel()
                    ToastUtil.showToast(this@ImVideoCallActivity, getString(R.string.f10))
                }
            }
        }
    }
    ...
    private fun startVideoFloatWindow(isAutoIntoBack: Boolean = false) {
        initWindow()
        ToastUtil.showToast(this, getString(R.string.f11))
        if (!isAutoIntoBack) {
            isManual = true
