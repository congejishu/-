...
class ImVoiceCallActivity : BaseActivity(), View.OnClickListener {
        ...
        if (!isRellyFinish && !isManual) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                if (FloatWindowUtil.checkFloatPermission(this@ImVoiceCallActivity)) {
                    ...
                    ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f13))
                }
            } else {
                startVoiceFloatWindow(true)
            }
    ...
    override fun onBackPressed() {
        ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f14))
    }
        ...
        when (v?.id) {
            R.id.img_answer -> {//接听
                if (NetworkUtils.isConnected()) {
                    ...
                    ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f15))
                    refuseCall()
                }
            }
            R.id.img_refuse -> {//拒接
            ...
            R.id.iv_float -> {//开启悬浮窗
                if (FloatWindowUtil.checkFloatPermission(this@ImVoiceCallActivity)) {
                    ...
                    ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f16))
                }
            }
        }
    }
    ...
    private val mRtcEventHandler = object : IRtcEngineEventHandler() { // Tutorial Step 1
        ...
        override fun onConnectionLost() {
            ...
            ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f17))
            mRtcEngine?.leaveChannel()
        }
    ...
    private var task: TimerTask = object : TimerTask() {
        override fun run() {
            ...
            runOnUiThread {
                if (!JitsistateMachine.isIncall) {
                    mRtcEngine?.leaveChannel()
                    ToastUtil.showToast(this@ImVoiceCallActivity, getString(R.string.f18))
                }
            }
        }
    }
    ...
    private fun startVoiceFloatWindow(isAutoIntoBack: Boolean = false) {
        ...
        ToastUtil.showToast(this, getString(R.string.f19))
        if (!isAutoIntoBack) {
            isManual = true
