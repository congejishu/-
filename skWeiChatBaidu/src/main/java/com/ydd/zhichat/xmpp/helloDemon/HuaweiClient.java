...
public class HuaweiClient implements HuaweiApiClient.ConnectionCallbacks, HuaweiApiClient.OnConnectionFailedListener {
    ...
    public void onConnected() {
        ...
        Log.v(TAG, "HuaweiApiClient 连接成功");
    ...
    public void onConnectionSuspended(int i) {
        ...
        Log.v(TAG, "HuaweiApiClient 连接断开");
    }

    @Override
    public void onConnectionFailed(ConnectionResult connectionResult) {
        Log.v(TAG, "HuaweiApiClient连接失败，错误码：" + connectionResult.getErrorCode());
    }
    ...
    public void getTokenAsyn() {
        ...
        tokenResult.setResultCallback(new ResultCallback<TokenResult>() {
            ...
            public void onResult(TokenResult result) {// 可至HuaweiPushRevicer内的onToken回调内查看Token结果，上传至服务器
                Log.v(TAG, "onResult() called with: result = [" + result + "]");
                Log.v("huaweiToken",result.getTokenRes().getToken());
                Log.v("huaweiToken",result.getTokenRes().getRetCode()+"");
                LogUtils.log(result);
            }
        });
    }
