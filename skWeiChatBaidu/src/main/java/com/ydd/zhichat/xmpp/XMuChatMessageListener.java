...
import static com.ydd.zhichat.pay.TransferMoneyDetailActivity.EVENT_SURE_RECEIPT;
...
public class XMuChatMessageListener implements MessageListener {
    ...
    private void saveGroupMessage(String body, String from, String packetId, boolean isDelay) {
        ...
        if (type == XmppMessage.TYPE_TRANSFER_RECEIVE) {
            ...
            chatMessage.setPacketId(chatMessage.getFilePath());
            if (TextUtils.isEmpty(chatMessage.getObjectId())) {
                Log.e("msg_muc", "Return 4");
                return;
            }
            ...
            chatMessage.setContent(chatMessage.getContent()+"|"+EVENT_SURE_RECEIPT);
            EventBus.getDefault().post(new EventTransfer(chatMessage));
            ChatMessageDao.getInstance().updateChatMessageReceiptStatus(mLoginUserId, roomJid, chatMessage.getPacketId());
