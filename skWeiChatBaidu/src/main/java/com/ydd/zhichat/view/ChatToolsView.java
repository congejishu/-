...
public class ChatToolsView extends RelativeLayout {
    ...
    private List<Item> dateList = new ArrayList<>();

    private void loadData() {
        // 隐藏个别按钮是通过删除指定drawable对应按钮实现，
        // 所以这里的drawable应该唯一，
        dateList.add(new Item(R.drawable.im_tool_photo_button_bg, R.string.chat_poto, () -> {
            listener.clickPhoto();
        },0));
        dateList.add(new Item(R.drawable.im_tool_camera_button_bg, R.string.chat_camera_record, () -> {
            listener.clickCamera();
        },1));
        if (CoreManager.requireConfig(MyApplication.getContext()).isAudiovisual != 3) {
            dateList.add(new Item(R.drawable.im_tool_local_button_bg, R.string.chat_with_video, () -> {
                ...
                if(CoreManager.requireConfig(MyApplication.getContext()).isAudiovisual == 0) {
                    contentView.findViewById(R.id.dialog_select_call_audio).setVisibility(VISIBLE);
                    contentView.findViewById(R.id.dialog_select_call_video).setVisibility(VISIBLE);
                }else if(CoreManager.requireConfig(MyApplication.getContext()).isAudiovisual == 1) {
                    contentView.findViewById(R.id.dialog_select_call_audio).setVisibility(GONE);//音频
                    contentView.findViewById(R.id.dialog_select_call_video).setVisibility(VISIBLE);//视频
                }else if(CoreManager.requireConfig(MyApplication.getContext()).isAudiovisual == 2) {
                    contentView.findViewById(R.id.dialog_select_call_audio).setVisibility(VISIBLE);//音频
                    contentView.findViewById(R.id.dialog_select_call_video).setVisibility(GONE);//视频
                }else{
                    contentView.findViewById(R.id.dialog_select_call_audio).setVisibility(GONE);
                    contentView.findViewById(R.id.dialog_select_call_video).setVisibility(GONE);
                }


            },2));
        }
        dateList.add(new Item(R.drawable.im_tool_loc_button_bg, R.string.chat_loc, () -> {
            listener.clickLocation();
        },3));
        dateList.add(new Item(R.drawable.im_tool_redpacket_button_bg, R.string.chat_redpacket, () -> {
            listener.clickRedpacket();
        },4));
        dateList.add(new Item(R.drawable.im_tool_transfer_button_bg, R.string.transfer_money, () -> {
            listener.clickTransferMoney();
        },5));
        dateList.add(new Item(R.drawable.im_tool_collection, R.string.chat_collection, () -> {
            listener.clickCollection();
        },6));
        dateList.add(new Item(R.drawable.im_tool_card_button_bg, R.string.chat_card, () -> {
            listener.clickCard();
        },7));
        dateList.add(new Item(R.drawable.im_tool_file_button_bg, R.string.chat_file, () -> {
            listener.clickFile();
        },8));
        dateList.add(new Item(R.drawable.im_contacts_button_norma, R.string.send_contacts, () -> {
            listener.clickContact();
        },9));
        dateList.add(new Item(R.drawable.im_tool_shake, R.string.chat_shake, () -> {
            listener.clickShake();
        },10));
        //群转账
        dateList.add(new Item(R.drawable.im_tool_shake, R.string.chat_shake, () -> {
    ...
    private void init(Context context) {
        ...
        for(int i = 0;i<dateList.size();i++) {
            Item item = dateList.get(i);
            if(item.type == 2) {
                if(MyApplication.mCoreManager.getConfig().vipConfig.getVoiceVideo() == 1) {
                    if(MyApplication.mCoreManager.getSelf().getVip() < 1) {
                        index.add(item);
                    }
                }
            }
...
//                    , R.drawable.im_tool_shake);
            ...
            gridPagerAdapter.removeAll(
                    ...
                    R.drawable.im_tool_redpacket_button_bg,//发红包
                    R.drawable.im_tool_local_button_bg
            );
             */
        } else {
...
class Item {
    ...
    public Item(int icon, int text, Runnable runnable,int type) {
        this.icon = icon;
        this.text = text;
