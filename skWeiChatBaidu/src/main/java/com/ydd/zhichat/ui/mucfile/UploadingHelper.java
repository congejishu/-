...
public class UploadingHelper {
    ...
    public static void upfile(String accessToken, String userId, final File file, final OnUpFileListener onUpFileListener) {
        try {
            if (!file.exists()) {
                onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.v12), file.getAbsolutePath());
            }

            if (TextUtils.isEmpty(userId)) {
                onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.v13), file.getAbsolutePath());
            }

            if (onUpFileListener == null) {
                onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.v14), file.getAbsolutePath());
            }
                ...
                public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {//数据发送成功，并且有返回值<不是上传成功。。。>
                    ...
                    if (arg0 == 200) {
                        ...
                        if (result == null || result.getResultCode() != Result.CODE_SUCCESS || result.getData() == null
                            ...
                            onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.v15), file.getAbsolutePath());
                        } else {
                    ...
                    if (TextUtils.isEmpty(url)) {
                        ...
                        onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.b90), file.getAbsolutePath());
                    } else {
            ...
            e.printStackTrace();
            onUpFileListener.onFailure(MyApplication.getInstance().getString(R.string.b89), file.getAbsolutePath());
        }
    }
