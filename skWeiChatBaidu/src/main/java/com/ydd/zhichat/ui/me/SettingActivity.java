...
//        mExitBtn.setBackground(new ColorDrawable(MyApplication.getContext().getResources().getColor(R.color.redpacket_bg)));
        ...
        if(coreManager.getConfig().isRestore) {
            findViewById(R.id.recover_rl).setVisibility(View.VISIBLE);
        }else{
            findViewById(R.id.recover_rl).setVisibility(View.GONE);
        }
        findViewById(R.id.recover_rl).setOnClickListener(this);
        //都可以跳转第三方
...
//            findViewById(R.id.tuisongmsg).setVisibility(View.GONE);
        ...
        if(MyApplication.mCoreManager.getConfig().vipConfig.getAutoReply() == 1) {
        ...
        switch (v.getId()) {
            ...
            case R.id.recover_rl:
                ...
                HttpUtils.get().url(coreManager.getConfig().GET_RECOVER)
                        .params(params)
                        .build()
                        .execute(new BaseCallback<RecoverActivity.RecoverV>(RecoverActivity.RecoverV.class) {

                            @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
                            @Override
                            public void onResponse(ObjectResult<RecoverActivity.RecoverV> result) {
                                DialogHelper.dismissProgressDialog();
                                if(result.getResultCode() == 1) {
                                    startActivity(new Intent(mContext, RecoverActivity.class));
                                }else{
...
*/
    ...
    private class ClearCacheAsyncTaska extends AsyncTask<Boolean, String, Integer> {
        ...
        protected void onPostExecute(Integer result) {
            ...
            LocaleHelper.setLocale(SettingActivity.this, LocaleHelper.getLanguage(SettingActivity.this));
