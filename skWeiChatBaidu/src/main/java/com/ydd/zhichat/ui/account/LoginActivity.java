...
public class LoginActivity extends BaseActivity implements View.OnClickListener {
    ...
    private Button forgetPasswordBtn, registerBtn, loginBtn ,loginBtnYouke;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initActionBar();
        initView();
        Log.e("hm--用户名登录",coreManager.getConfig().registerUsername+"");
    ...
    private void initView() {
       ...
       if (coreManager.getConfig().registerUsername) {
        ...
        if(coreManager.getConfig().isOpenYouke) {
            findViewById(R.id.yk_login_btn).setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            findViewById(R.id.yk_login_btn).setVisibility(View.GONE);
        }
...
//                startActivityForResult(intent, SelectPrefixActivity.REQUEST_MOBILE_PREFIX_LOGIN);
    ...
    public int getNum(int endNum){
     ...
     private  void loginYouke(boolean third){
        int phonerandom  = getNum(10000001);
         String phoneStr = android.provider.Settings.Secure.getString(this.getContentResolver(), android.provider.Settings.Secure.ANDROID_ID);;//String.format("%d", phonerandom);
         RegisterUserBasicInfoActivity.start(
                 this,
                 "" + mobilePrefix,
                 phoneStr,
                 Md5Util.toMD5("123456"),
                 "".toString(),
                 thirdToken,
                 "youke"
         );
                ...
                .execute(new BaseCallback<LoginRegisterResult>(LoginRegisterResult.class) {
                    ...
                    public void onResponse(com.xuan.xuanhttplibrary.okhttp.result.ObjectResult<LoginRegisterResult> result) {
                        ...
                        if (success) {
                            ...
                            if(result.getData().getPasswordMode() == 1) {
                                //清空数据
                                List<Friend> mNearChatFriendList = FriendDao.getInstance().getNearlyFriendMsg(result.getData().getUserId());
                                for (int i = 0; i < mNearChatFriendList.size(); i++) {
                                    FriendDao.getInstance().resetFriendMessage(result.getData().getUserId(), mNearChatFriendList.get(i).getUserId());
                                    ChatMessageDao.getInstance().deleteMessageTable(result.getData().getUserId(), mNearChatFriendList.get(i).getUserId());
                                }
                            }
